...
from easy_pipeline.main_ease import EASERecommendationSystem
...
def get_top_pop() -> DbTopPopular:
    ...
    Вызывается один раз за запуск приложения.
    """
    system = DbTopPopular(K_RECS)
    return system


def get_recommendations(user_id, cart_product_ids, exclude_cart: bool = True):
    """
    ...
    raw_recs = system.get_recommendations(
        user_activity=list(cart_product_ids), top_k=K_RECS
    )
    print(f"{raw_recs = }\n\n\n")
